...
const normT = (t) => String(t || '').toLowerCase().replace(/[\s\W]+/g, '');

// 제목을 의미 토큰으로 분해([단독]류 대괄호·기호 제거, 2글자 이상만).
const STOP = new Set(['프로농구', '농구', '경기', '기자', '오늘', '이날', '지난', '대한', '위해', '관련', '오후', '오전']);
...
// 다른 매체가 같은 사건을 다룬 '유사 중복'인가 — 유사도 0.55↑이고 공통토큰 3개↑면 중복.
function isSimilar(tokens, keptSets) {
  const set = new Set(tokens);
  for (const k of keptSets) {
    let inter = 0;
    for (const x of set) if (k.has(x)) inter++;
    if (inter >= 3 && inter / (set.size + k.size - inter) >= 0.55) return true;
  }
  return false;
}

export function buildNewsEmail(news, opts = {}) {
  const { groups, generatedAt } = news;
  const yongsanKeywords = opts.yongsanKeywords || []; // 순서=용산 소식 챕터 내 정렬 우선순위
  const order = opts.order || [];                     // 일반 챕터 표시 순서
  const isYongsan = (kw) => yongsanKeywords.includes(kw);
  const dateStr = kstDateStr(generatedAt);

  // 1) 일반 챕터를 지정 순서로 정렬. order가 있으면 '허용목록'으로도 작동(목록에 없는 챕터는 제외).
  const orderIdx = (kw) => { const i = order.indexOf(kw); return i < 0 ? 999 : i; };
  const mainGroups = groups
    .filter((g) => g.items.length && !isYongsan(g.keyword) && (order.length === 0 || order.includes(g.keyword)))
    .sort((a, b) => orderIdx(a.keyword) - orderIdx(b.keyword) || a.keyword.localeCompare(b.keyword));
  // 용산 그룹은 yongsanKeywords 우선순위 순으로
  const yGroups = yongsanKeywords.map((k) => groups.find((g) => g.keyword === k)).filter((g) => g && g.items.length);

  // 2) 전역 중복 제거 — 제목 완전일치 + '다른 매체의 같은 기사'(유사도) 모두 제거. 앞 챕터 우선.
  const seen = new Set();
  const keptSets = [];
  const dedup = (items) => items.filter((it) => {
    const k = normT(it.title);
    if (!k || seen.has(k)) return false;
    const toks = titleTokens(it.title);
    if (isSimilar(toks, keptSets)) return false; // 다른 매체 동일 사건
    seen.add(k); keptSets.push(new Set(toks));
    return true;
  });
  const mainDedup = mainGroups.map((g) => ({ keyword: g.keyword, items: dedup(g.items) })).filter((g) => g.items.length);
  const yongsanItems = [];
  for (const g of yGroups) for (const it of dedup(g.items)) yongsanItems.push(it);

  const total = mainDedup.reduce((n, g) => n + g.items.length, 0) + yongsanItems.length;
  const subject = `[농구 뉴스] ${dateStr} · ${total}건`;

  const sections = mainDedup.map((g) => {
  ...
  // 하단 '용산 소식' 챕터 — 용산중·용산고·신석 코치·이정석 코치 순.
  const yongsanSection = yongsanItems.length
    ? `<div style="margin-top:26px;border-top:2px solid #1F3864;padding-top:14px">
        <div style="font-size:16px;font-weight:800;color:#1F3864">🏫 용산 소식 <span style="font-size:12px;font-weight:600;color:#888">· 용산중·용산고·신석 코치·이정석 코치</span></div>
