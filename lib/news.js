...
// 여러 검색 키워드 그룹을 하나의 표시 그룹으로 병합(예: 중고/고교/중학/여고/여중 → '중고 농구').
// 검색은 세분화해서 하되 메일·페이지에선 한 챕터로 보여주고 싶을 때 사용.
export function mergeGroups(groups, mergeKeywords, label) {
  if (!mergeKeywords?.length || !label) return groups;
  const merged = { keyword: label, items: [] };
  const rest = [];
  for (const g of groups) {
    if (mergeKeywords.includes(g.keyword)) merged.items.push(...g.items);
    else rest.push(g);
  }
  merged.items.sort((a, b) => b.ts - a.ts);
  // 병합 그룹은 원래 첫 병합 키워드가 있던 자리에 배치
  const firstIdx = groups.findIndex((g) => mergeKeywords.includes(g.keyword));
  if (firstIdx < 0) return groups;
  const out = [];
  let inserted = false;
  for (let i = 0; i < groups.length; i++) {
    const g = groups[i];
    if (mergeKeywords.includes(g.keyword)) {
